...
def _validate_preprocessing_config(
    ...
    invalid_requested = sorted(
        preproc_id
        for preproc_id in (preprocessing_ids or [])
        ...
        *,
        ...
        return DatasetPreparationConfig(
            ...
            augmentations_per_image=(
                self.preprocess.augmentations_per_image
...
def load_experiment_config(path: str | Path | None = None) -> ExperimentConfig:
    ...
    preprocessing_grid = {
        preproc_id: _normalize_grid_values(param_space)
        for preproc_id, param_space in raw_preprocessing_grid.items()
    }
    model_config = {
        model_name: _model_runtime_from_dict(model_values)
        for model_name, model_values in raw_config["models"].items()
    }
