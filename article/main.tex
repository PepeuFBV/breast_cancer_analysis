...
\subsection*{2.2. Data Augmentation}
...
After augmentation, the dataset was split into training and test sets using an 80/20 ratio, maintaining the class distribution (stratified split). Only images with valid BI-RADS labels (1, 2, 3, 4a, 4b, 4c, 5, 6) were retained for further analysis (no images were discarded).

This augmentation strategy significantly increased the effective dataset size and diversity, from 410 to y, helping to prevent overfitting and improve the robustness of the trained models. The resulting dataset was used as input for the subsequent preprocessing and model training steps described in the following sections.
...
Each preprocessing function was applied to the dataset as a separate experiment, and all possible pairs of transformations (in both orders) were also tested to assess the effect of combining techniques. This systematic approach allowed us to identify which preprocessing strategies most effectively enhance model performance, while keeping the computational requirements low and the pipeline easily reproducible.
